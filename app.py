...
# A quick jump menu — since everything now lives on one scrollable page,
# this just lets people click straight to a section instead of scrolling.
st.markdown(
    "**Jump to:** "
...
# 1. Preprocessing
# ---------------------------------------------------------------------------
with st.expander("1️⃣ Preprocessing", expanded=True):
    st.subheader("Tokenization, Stopword Removal, Stemming & Lemmatization", anchor="preprocessing")
    text = st.text_area(
        "Enter text",
...
# 2. Vectorization
# ---------------------------------------------------------------------------
with st.expander("2️⃣ Vectorization", expanded=True):
    st.subheader("Bag-of-Words vs TF-IDF", anchor="vectorization")
...
# 3. POS & NER
# ---------------------------------------------------------------------------
with st.expander("3️⃣ POS & NER", expanded=True):
    st.subheader("Part-of-Speech Tagging & Named Entity Recognition", anchor="pos-ner")
...
# ---------------------------------------------------------------------------
with st.expander("4️⃣ Naive Bayes Classifier", expanded=False):
    st.subheader("Naive Bayes Sentiment Classifier (toy example)", anchor="naive-bayes-classifier")
    st.write(
...
# 5. Word2Vec
# ---------------------------------------------------------------------------
with st.expander("5️⃣ Word2Vec", expanded=False):
    st.subheader("Word2Vec Embeddings (gensim)", anchor="word2vec")
...
# 6. LSTM model
# ---------------------------------------------------------------------------
with st.expander("6️⃣ LSTM Model", expanded=False):
    st.subheader("LSTM Classifier Architecture (PyTorch)", anchor="lstm-model")
    st.write(
...
# ---------------------------------------------------------------------------
with st.expander("7️⃣ Transformers", expanded=False):
    st.subheader("Pretrained Transformer Pipelines (Hugging Face)", anchor="transformers")
    st.warning(
